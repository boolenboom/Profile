...
let portfolioShow = ref(false);

function show(entries){
    portfolioShow.value = entries[0].isIntersecting;
}

function initial(){
    let ob = new IntersectionObserver(show,{threshold:0.99});
    let watchDom = document.querySelector('#portfolio');
    ob.observe(watchDom);
}
onMounted(()=>{
    initial();
...
<template>
    <div id="portfolio">
        <RadialSlider :imageList="imageList" :contentAmount="4" :class="`${portfolioShow ? 'pass' : 'initial'}`">
            <template #slide1>
                <article>
                    <div class="workImg">
